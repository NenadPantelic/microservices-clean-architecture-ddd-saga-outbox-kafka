...
public class OrderController {
    ...
    public ResponseEntity<CreateOrderResponse> createOrder(@RequestBody CreateOrderCommand createOrderCommand) {
        log.info("Creating an order for customer[id = {}] and restaurant[id = {}]",
                createOrderCommand.customerId(), createOrderCommand.restaurantId()
        );
        CreateOrderResponse createOrderResponse = orderApplicationService.createOrder(createOrderCommand);
        log.info("Order created[id = {}]", createOrderResponse.orderTrackingId());
    ...
    public ResponseEntity<TrackOrderResponse> getOrderByTrackingId(@PathVariable("trackingId") String trackingId) {
        ...
        log.info("Found the tracking order by id {}", trackOrderResponse.orderTrackingId());
